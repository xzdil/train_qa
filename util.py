...
def visualize(tbx, pred_dict, gold_dict, step, split, num_visuals):
    ...
    for i, id_ in enumerate(visual_ids):
        ...
        answers = gold_dict["answers"]["text"][idx_gold_dict]
...
def read_squad(path):
    ...
    data_dict = {"question": [], "context": [], "id": [], "answers": []}
    ...
    if data_dict["answers"]:
        data_dict_collapsed["answers"] = []
    for qid in id_map:
        ex_ids = id_map[qid]
        ...
        if data_dict["answers"]:
            all_answers = [data_dict["answers"][idx] for idx in ex_ids]
            data_dict_collapsed["answers"].append(
                {
...
def eval_dicts(gold_dict, pred_dict):
    ...
    for curr_id in pred_dict:
        ...
        ground_truths = gold_dict["answers"]["text"][index]
...
def postprocess_qa_predictions(
    ...
    for example_index in tqdm(range(len(examples["id"]))):
        ...
        example = {key: examples[key][example_index] for key in ["id","context"]}
        #example = {key: examples.get(key, {})[example_index] for key in ["id","context","question","answers"]}
