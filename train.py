...
def prepare_train_data(dataset_dict, tokenizer):
    ...
    for i, offsets in enumerate(tqdm(offset_mapping)):
        ...
        start_char = answer["answer_start"][sample_index]
        end_char = start_char + len(answer["text"][sample_index])
        tokenized_examples["id"].append(dataset_dict["id"][sample_index])
        ...
        if not (
            ...
            if context[offset_st:offset_en] != answer["text"][sample_index]:
                inaccurate += 1
...
class Trainer:
    ...
    def save(self, model, best_scores):
        print(best_scores)
        if self.model_type in ["distilbert","loaddis"]:
            # model.save_pretrained(self.path)
            
            f1_score = best_scores["F1"]
            torch.save(model.state_dict(), self.path + f"/QazDistilBERT.pt")
            print("DistilBERT saved as ", self.path + f"/QazDistilBERT.pt")
        elif self.model_type in ["moe","load"]:
            # model.save_pretrained(self.path)
            #f1_score = best_scores["F1"]
            #torch.save(model.state_dict(), self.path + f"/model_moe_f1_{f1_score}.pt") # Это рекомендованный метод
            torch.save(model, self.path + f"/model_moe_final.pt")
            print("MoE saved as ", self.path + f"/model_moe_final.pt")
        elif self.model_type in ["switch_transformer","loadswitch"]:
            # model.save_pretrained(self.path)
            #f1_score = best_scores["F1"]
            #torch.save(model.state_dict(), self.path + f"/model_moe_f1_{f1_score}.pt") # Это рекомендованный метод
            torch.save(model, self.path + f"/switch_transformer.pt")
            print("switch_transformer saved as ", self.path + f"/switch_transformer.pt")
        else:
        ...
        for epoch_num in range(self.num_epochs):
            ...
            with torch.enable_grad(), tqdm(
                ...
                for batch in train_dataloader:
                    ...
                    if (global_idx >= global_idx_count * 100) and rank == 0:
            ...
            for epoch_num in range(self.num_epochs_pretrain):
                ...
                with torch.enable_grad(), tqdm(
                    ...
                    for batch in pretrain_dataloader:
                        ...
                        if (
                            pretrain_step_idx >= pretrain_eval_count * 100
                        ) and rank == 0:
                            ...
                            for k, v in curr_score.items():
                                tbx.add_scalar(
                                    f"pretrain_val/{k}", v, pretrain_step_idx
                                )
            ...
            with torch.enable_grad(), tqdm(
                ...
                for batch in train_dataloader:
                    ...
                    if (global_idx >= global_idx_count * self.eval_every) and rank == 0:
                        ...
                        if curr_score["F1"] >= best_scores["F1"]:
                            best_scores = curr_score
                            self.save(model, best_scores) # хз почему убрали, мб не работает
                            self.log.info("Infer on testset...")
                            self.test_moe(
                                model,
                                test_dataloader,
...
def get_dataset(args, tokenizer, path, split_name, num_aug=0):
    ...
    for json_str in dataset:
        ...
        dataset_dict['answers']["text"].append(result['answers']['text'][0])
        dataset_dict['answers']["answer_start"].append(result['answers']['answer_start'][0])
    ...
    data_encodings = read_and_process(
            args, tokenizer, dataset_dict, "sagemaker-studiolab-notebooks/cache", split_name
        )
...
def main(rank, world_size, args):
    assert args.model_type in [
        ...
        "load",
        "loaddis"
    ], "model must be either distilbert, moe, or switch_transformer"
    ...
    if torch.cuda.is_available():
        ...
        print("CUDA is not detected, using cpu")
        device = "cpu" 
    util.set_seed(args.seed)
    if rank == 0:
        run = wandb.init(project="QAMOE")
    else:
        run = None

    if args.model_type == "distilbert":
        model = DistilBertForQuestionAnswering.from_pretrained("distilbert/distilbert-base-multilingual-cased").to(rank)
        #model = DistilBertForQuestionAnswering(DistilBertConfig.from_pretrained('distilbert/distilbert-base-multilingual-cased')).to(rank)
    elif args.model_type == "loaddis":
        #model = DistilBertForQuestionAnswering(DistilBertConfig.from_pretrained('distilbert/distilbert-base-multilingual-cased')).to(rank)
        model = torch.load("save/best_f1_18_3_loss_2_7/checkpoint/QazDistilBERT.pt")
        #model.load_state_dict(state_dict)
        print("Distilbert from state dict loaded!")
    elif args.model_type == "load":
        model = torch.load("save/pretrain_moe-efbc32e1-1e82-46fb-959c-009607526d2b/checkpoint/model_moe_final.").to(rank)
        print("Model loaded!")
        ...
        st_layer = SwitchTransformerLayer(
            d_model=args.dim, attn=attn, feed_forward=st_ff, dropout_prob=0.2
        )
        model = SwitchTransformer(
            ...
            n_experts=args.num_experts,
            device=device,
            load_balancing_loss_ceof=args.load_balancing_loss_ceof,
        ).to(rank)
        ...
        raise ValueError(
            "model_type must be either distilbert, moe, or switch_transformer"
        )
    if world_size > 1:
    ...
    if args.do_train:
        ...
        if args.pretrain:
            ...
            pretrain_dataset, _ = get_dataset(
                args, tokenizer, "dataset.jsonl", "train", args.num_aug_pretrain
            )
            train_dataset, _ = get_dataset(args, tokenizer,"train.jsonl", "finetune", args.num_aug)
        else:
            # No finetuning dataset, only one train dataset
            train_dataset, _ = get_dataset(args, tokenizer,"dataset.jsonl", "train", args.num_aug)
        ...
        if rank == 0:
            ...
            ood_val_dataset, ood_val_dict = get_dataset(args, tokenizer,"val.jsonl", "ood_val")
            ...
            test_dataset, test_dict = get_dataset(args, tokenizer, "test.jsonl", "test")
        ...
        if args.model_type in ["distilbert",'loaddis']:
        ...
        elif args.model_type in ["moe", "switch_transformer","load"]:
    ...
    if args.do_eval:
        ...
        eval_preds, eval_scores = trainer.evaluate(
            model, eval_loader, eval_dict, return_preds=True, split=split_name
        )
        results_str = ", ".join(f"{k}: {v:05.2f}" for k, v in eval_scores.items())
